...
Solution <- setClass("Solution",
                              ...
                              validity = function(object) {
                                object@status %in% c("infeasible", "unbounded", "optimal") &&
...
setMethod("get_solution",
          ...
          definition = function(solution, exp) {
            ...
            if (is_indexed_var) {
              ...
              instance_pattern <- paste0(var_name, "\\[", paste0(idx_pattern, collapse = ","), "\\]")
              if (length(free_vars) == 0) {
                return(solution@solution[grepl(x = names(solution@solution), pattern = instance_pattern)])
              } else {
                # the solution is sorted lexigographically
                var_index <- stringr::str_match(names(solution@solution), pattern = instance_pattern)
                na_rows <- as.logical(apply(is.na(var_index), 1, all))
                var_index <- var_index[!na_rows, ]
                var_values <- solution@solution[grepl(names(solution@solution), pattern = instance_pattern)]
              ...
              if (!var_name %in% names(solution@solution)) {
                stop("Either variable is not part of the model or you have to specify the indexes.")
              }
