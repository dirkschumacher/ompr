...
The last constraint uses *filter constraints*:
```{r, eval=FALSE}
add_constraint(x[i, k] + x[j, k] <= 1, i = 1:n, j = 1:n, k = 1:max_colors, is_adjacent(i, j))
```
This code generates a constraint for each combination of i, j and k with the additional constraint, that `is_adjacent(i, j)` needs to evaluate to `TRUE`. Using *filter constraints* let's you design a very specific set of constraints with minimal code. This principle can be used with all functions that acceppt quantifiers: `sum_expr`, `set_bounds`, `add_constraints` and `add_variable`.
