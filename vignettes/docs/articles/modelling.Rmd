...
model <- MILPModel() %>% 
  ...
  set_objective(sum_exp(v[i] * x[i], i = 1:n)) %>% 
  add_constraint(sum_exp(w[i] * x[i], i = 1:n) <= W)
```
...
MILPModel() %>% 
  ...
  add_constraint(sum_exp(x[i], i = 1:3) <= 10)
```

## Quantifiers

`add_variable`, `add_constraint`, `set_bounds`, `sum_exp` all support a common quantifier interface that also supports filter expression. A more complex example will show what that means.
...
MILPModel() %>% 
  ...
  set_objective(sum_exp(x[i, j], i = 1:10, j = 1:10, i == j + 1)) %>% 
  
  # for each i between 1 and 10 with i mod 2 = 0
  # we add a constraint \sum_j x_{i,j}
  add_constraint(sum_exp(x[i, j], j = 1:10) <= 1, i = 1:10, i %% 2 == 0) %>% 
  
  # of course you can leave out filters or add more than 1
  add_constraint(sum_exp(x[i, j], j = 1:10) <= 2, i = 1:10) 
```
...
MILPModel() %>% 
  ...
  set_objective(sum_exp(x[i, j], i = 1:10, j = 1:10)) %>% 
...
MILPModel() %>% 
  ...
  set_objective(sum_exp(cost[i] * x[i], i = 1:n)) %>% 
  add_constraint(sum_exp(x[i], i = 1:n) <= max_elements)
```
...
result <- MILPModel() %>% 
  ...
  set_objective(sum_exp(weights[i, j] * x[i, j], i = 1:n, j = 1:n)) %>% 
